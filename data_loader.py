...
Handles CSV loading with separate train/test files and Reentrancy label mapping
"""
...
class VulnerabilityDataset:
    ...
    def print_statistics(self):
        ...
        for label_name, stats in dist.items():
            logger.info(f"  - {label_name}: {stats['count']} ({stats['percentage']:.2f}%)")
        ...
        logger.info(f"\nMulti-label statistics:")
        logger.info(f"  - Samples with at least 1 vulnerability: {samples_with_labels} ({samples_with_labels/len(self)*100:.2f}%)")
        logger.info(f"  - Samples with multiple vulnerabilities: {samples_with_multiple} ({samples_with_multiple/len(self)*100:.2f}%)")
        logger.info(f"{'='*70}\n")
...
def load_reentrancy_mapping() -> Dict[str, int]:
    ...
    """
    logger.info(f"Loading Reentrancy mapping from {config.REENTRANCY_MAPPING_PATH.name}")
    
    if not config.REENTRANCY_MAPPING_PATH.exists():
        logger.warning(f"Reentrancy mapping file not found: {config.REENTRANCY_MAPPING_PATH}")
        return {}
...
@timeit
def load_dataset_from_file(file_path: Path, reentrancy_mapping: Dict[str, int]) -> VulnerabilityDataset:
    """
    Load dataset from a single CSV file (train or test)
    
    Args:
        file_path: Path to CSV file
        reentrancy_mapping: Dictionary mapping address to Reentrancy label
    ...
    with Timer(f"Loading {file_path.name}", logger):
        ...
        required_cols = [config.DATASET_COLUMNS['address'], config.DATASET_COLUMNS['ast']]
        
        # Add label columns (excluding Reentrancy which we'll map separately)
        label_cols = [col for col in config.DATASET_COLUMNS['labels'] if col != 'Reentrancy']
    ...
    # Get labels (multi-label) - first get the 4 labels from the file
    labels_list = []
    for col in config.DATASET_COLUMNS['labels']:
        if col == 'Reentrancy':
            # Map Reentrancy from DASP v2
            reentrancy_labels = []
            for addr in addresses:
                addr_lower = addr.strip().lower()
                reentrancy_labels.append(reentrancy_mapping.get(addr_lower, 0))
            labels_list.append(reentrancy_labels)
        else:
            # Get from current dataframe
...
@timeit
def load_train_test_datasets(force_reload: bool = False) -> Tuple[VulnerabilityDataset, VulnerabilityDataset]:
    """
    Load train and test datasets from separate CSV files
    ...
    if not force_reload and pickle_exists(config.PICKLE_TRAIN_DATASET) and pickle_exists(config.PICKLE_TEST_DATASET):
    ...
    # Load Reentrancy mapping first
    reentrancy_mapping = load_reentrancy_mapping()
    
    # Load train dataset
    train_dataset = load_dataset_from_file(config.TRAIN_DATASET_PATH, reentrancy_mapping)
    
    # Load test dataset
    test_dataset = load_dataset_from_file(config.TEST_DATASET_PATH, reentrancy_mapping)
...
def get_label_names() -> List[str]:
    ...
    return [config.LABEL_DISPLAY_NAMES.get(l, l) for l in config.DATASET_COLUMNS['labels']]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sample', type=int, help='Sample number to view (1-based index)')
    ...
    parser.add_argument('--max-ast-chars', type=int, default=500, help='Max AST characters to show')
    parser.add_argument('--range', type=str, help='Sample range to analyze (e.g., "1-100")')
    ...
    if args.sample is not None:
        ...
        active_labels = [label_names[i] for i, val in enumerate(sample['labels']) if val == 1]
    ...
    # If range specified
        ...
        try:
            ...
            logger.info(f"\n{'='*70}")
            logger.info(f"Analyzing Samples {start}-{end} from {args.dataset.upper()} set")
            logger.info(f"{'='*70}")
            ...
            logger.info(f"\n  Multi-label Statistics:")
            logger.info(f"    - Samples with 0 vulnerabilities: {np.sum(samples_with_labels == 0)}")
            logger.info(f"    - Samples with 1 vulnerability: {np.sum(samples_with_labels == 1)}")
            logger.info(f"    - Samples with 2+ vulnerabilities: {np.sum(samples_with_labels >= 2)}")
            logger.info(f"    - Max vulnerabilities per sample: {int(np.max(samples_with_labels))}")
            logger.info(f"    - Avg vulnerabilities per sample: {np.mean(samples_with_labels):.2f}")
    ...
    # Default: show overall dataset statistics
        ...
        for dataset_obj, dataset_name in [(train_dataset, "Train"), (test_dataset, "Test")]:
            logger.info(f"\n  {dataset_name} Set:")
            
            # Check addresses
            null_addresses = sum(1 for addr in dataset_obj.addresses if not addr or addr == "")
            ...
            logger.info(f"    - AST size range: {min(ast_sizes):,} - {max(ast_sizes):,} characters")
        ...
        logger.info(f"\n  Multi-label Statistics:")
        logger.info(f"    - Samples with at least 1 vulnerability: {np.sum(train_samples_with_labels > 0)} ({(np.sum(train_samples_with_labels > 0) / len(train_dataset) * 100):.1f}%)")
        logger.info(f"    - Samples with multiple vulnerabilities: {np.sum(train_samples_with_labels > 1)} ({(np.sum(train_samples_with_labels > 1) / len(train_dataset) * 100):.1f}%)")
        logger.info(f"    - Average vulnerabilities per sample: {np.mean(train_samples_with_labels):.2f}")
        ...
        logger.info(f"\n  Multi-label Statistics:")
        logger.info(f"    - Samples with at least 1 vulnerability: {np.sum(test_samples_with_labels > 0)} ({(np.sum(test_samples_with_labels > 0) / len(test_dataset) * 100):.1f}%)")
        logger.info(f"    - Samples with multiple vulnerabilities: {np.sum(test_samples_with_labels > 1)} ({(np.sum(test_samples_with_labels > 1) / len(test_dataset) * 100):.1f}%)")
        logger.info(f"    - Average vulnerabilities per sample: {np.mean(test_samples_with_labels):.2f}")
        ...
        logger.info(f"  - View test set: python data_loader.py --sample 5 --dataset test")
