...
class GNNClassifier(nn.Module):
    ...
    def __init__(self, num_node_features: int, hidden_channels: int = 64, num_classes: int = 4):
...
def compute_node_importance_simple(nx_graph: nx.DiGraph) -> Dict[any, float]:
    """
    Compute node importance using graph-based heuristics
    (Fallback method when GNN Explainer is not applicable)
    ...
    try:
        ...
        for node in nx_graph.nodes():
            score = (
                ...
                0.6 * betweenness.get(node, 0)
            )
            importance_scores[node] = score
    except:
        # Fallback: use degree only
        for node in nx_graph.nodes():
            degree = nx_graph.degree(node)
            importance_scores[node] = degree / max(1, nx_graph.number_of_nodes())
...
def optimize_single_graph(
    ...
    if use_gnn_explainer and gnn_model is not None:
        ...
        # Map to node IDs
        node_list = list(nx_graph.nodes())
        node_importance = {node: float(node_importance_array[i]) for i, node in enumerate(node_list)}
    else:
    ...
    important_nodes = [node for node, score in node_importance.items() if score >= threshold]
...
def optimize_graphs(
    ...
    if not force_reoptimize and pickle_exists(config.PICKLE_OPTIMIZED_GRAPHS):
        logger.info(f"Loading optimized graphs from cache: {config.PICKLE_OPTIMIZED_GRAPHS.name}")
    ...
    logger.info(f"Optimizing {len(cfg_graphs)} graphs using {'GNN Explainer' if use_gnn_explainer else 'simple heuristics'}...")
    ...
    if use_gnn_explainer and HAS_TORCH_GEOMETRIC:
        ...
        gnn_model = GNNClassifier(num_node_features=4, num_classes=len(config.DATASET_COLUMNS['labels']))
        gnn_model.eval()
    elif use_gnn_explainer and not HAS_TORCH_GEOMETRIC:
        logger.warning("GNN Explainer requested but torch-geometric not available, using heuristics")
        use_gnn_explainer = False
    ...
    with Timer("Graph optimization", logger):
        for i, cfg_graph in enumerate(cfg_graphs):
            ...
            try:
                optimized = optimize_single_graph(cfg_graph, gnn_model, use_gnn_explainer)
    ...
    logger.info(f"Saving optimized graphs to cache: {config.PICKLE_OPTIMIZED_GRAPHS.name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sample', type=int, default=1, help='Sample number to optimize (1-based index)')
    parser.add_argument('--dataset', choices=['train', 'test'], default='train', help='Dataset to use')
    parser.add_argument('--method', choices=['gnn', 'heuristic'], default='heuristic', 
                       help='Optimization method (GNN Explainer or heuristic)')
    parser.add_argument('--show-nodes', action='store_true', help='Show which nodes were kept/removed')
    parser.add_argument('--threshold', type=float, help='Custom SHAP threshold (default from config)')
    ...
    logger.info(f"  Nodes removed: {cfg_graph.graph.number_of_nodes() - optimized_graph.graph.number_of_nodes()}")
    
    reduction_pct = 100 * (1 - optimized_graph.graph.number_of_nodes() / cfg_graph.graph.number_of_nodes()) if cfg_graph.graph.number_of_nodes() > 0 else 0
    ...
    logger.info(f"  Edges removed: {cfg_graph.graph.number_of_edges() - optimized_graph.graph.number_of_edges()}")
    ...
    if args.show_nodes:
        ...
        if removed_nodes:
            ...
            sorted_types = sorted(removed_types.items(), key=lambda x: x[1], reverse=True)
    ...
    # Calculate densities
    orig_density = cfg_graph.graph.number_of_edges() / (cfg_graph.graph.number_of_nodes() * (cfg_graph.graph.number_of_nodes() - 1)) if cfg_graph.graph.number_of_nodes() > 1 else 0
    opt_density = optimized_graph.graph.number_of_edges() / (optimized_graph.graph.number_of_nodes() * (optimized_graph.graph.number_of_nodes() - 1)) if optimized_graph.graph.number_of_nodes() > 1 else 0
    
    logger.info(f"  Original density: {orig_density:.4f}")
    logger.info(f"  Optimized density: {opt_density:.4f}")
    
    # Average degrees
    orig_avg_deg = sum(dict(cfg_graph.graph.degree()).values()) / cfg_graph.graph.number_of_nodes() if cfg_graph.graph.number_of_nodes() > 0 else 0
    opt_avg_deg = sum(dict(optimized_graph.graph.degree()).values()) / optimized_graph.graph.number_of_nodes() if optimized_graph.graph.number_of_nodes() > 0 else 0
